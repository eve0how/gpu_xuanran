...
清华大学 PA1 光线追踪作业：在课程框架上实现 Whitted 追踪、路径追踪、Cook-Torrance 光泽、NEE/MIS、色散、纹理与 CUDA/OpenMP 加速。
...
| `raytracer.hpp` | **CPU 渲染核心**（header-only） | `RayTracer`、`RenderMode`、`trace()`、`castRayWhitted()`、`castRayPath()`、NEE/MIS/色散 |
...
| 文件 | 作用 | 关键类/函数 |
|------|------|-------------|
| `main.cpp` | 程序入口：解析 CLI、加载场景、CPU OpenMP / CUDA 渲染循环、保存 BMP | `main()`、`parseMode()`、`hash01()` 抖动采样 |
...
| `scene_whitted.txt` | 完整 Cornell Whitted（报告常用） |
...
| `scene_mis_demo.txt` | **MIS 三方对比**：小面光 + 近镜面光泽球 |
| `scene_showcase.txt` | 综合展示：Cornell + 玻璃 cube（色散） |
| `scene_dispersion.txt` | 棱镜色散（与 `scene_prism.txt` 等价） |
| `scene_prism.txt` | 三角棱镜 `prism.obj`，天花板狭缝 → 后墙彩虹 |
| `scene_prism_darkside.txt` | 暗室棱镜演示（侧光 + 投影屏） |
...
| `textured_cube.obj` | 备用带 UV 立方体 |

### output/ 与 results/

| 目录 | 用途 |
|------|------|
| `output/` | `run_all.sh` / 手动渲染的输出根目录 |
...
*基于当前 `code/` 源码整理；`include/` 与 `src/` 下每个源文件均已列出。*
