...
| `scene_texture_mesh.txt` | 纹理网格（砖/木 albedo，`vt` 插值） |
| `scene_texture_bonus.txt` | **加分验收**：砖纹地板 mesh + 地球纹理球；对照 `scene_texture_bonus_notex.txt` |
...
| `cornell_floor.obj` / `cornell_left_wall.obj` / `cornell_back_wall.obj` | Cornell 分墙网格（纹理场景，`vt`） |
| `uv_sphere.obj` | 低模 UV 球（`vt`+`vn`，可选 mesh 球贴图演示） |
