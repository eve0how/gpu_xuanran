...
// Cook-Torrance microfacet BRDF (PPT slides 53-56):
// f = kd * rho_d/pi + ks * D*G*F / (4*(n·wi)*(n·wo))
// D: Beckmann NDF (roughness m), G: Cook-Torrance geometry, F: Schlick Fresnel.
struct CookTorranceBRDF {
    ...
    static float cookTorranceG1(const Vector3f &n, const Vector3f &w, const Vector3f &h) {
        float nw = std::max(0.0f, Vector3f::dot(n, w));
        float nh = std::max(0.0f, Vector3f::dot(n, h));
        float wh = std::max(0.001f, Vector3f::dot(w, h));
        return std::min(1.0f, 2.0f * nh * nw / wh);
    }

    static float cookTorranceG(const Vector3f &n, const Vector3f &wo, const Vector3f &wi,
                               const Vector3f &h) {
        return cookTorranceG1(n, wo, h) * cookTorranceG1(n, wi, h);
    }
    ...
    static Vector3f evaluateSpecular(const Vector3f &n, const Vector3f &wo, const Vector3f &wi,
        ...
        float G = cookTorranceG(n, wo, wi, h);
...
public:
    RefractMaterial(const Vector3f &color, float index, float dispersion = 0.02f) :
