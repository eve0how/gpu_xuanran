...
Light *SceneParser::parseAreaLight() {
    ...
    getToken(token);
    assert(!strcmp(token, "}"));
    return new AreaLight(v0, v1, v2, color);
}
// ====================================================================
// ====================================================================
...
void SceneParser::parseMaterials() {
    ...
    while (num_materials > count) {
        ...
        } else if (!strcmp(token, "ReflectMaterial") ||
                   !strcmp(token, "ReflectiveMaterial")) {
            materials[count] = parseReflectMaterial();
        } else if (!strcmp(token, "RefractMaterial") ||
                   !strcmp(token, "RefractiveMaterial")) {
            materials[count] = parseRefractMaterial();
...
Material *SceneParser::parseReflectMaterial() {
    char token[MAX_PARSER_TOKEN_LENGTH];
    Vector3f reflectColor(1, 1, 1);
    getToken(token);
    assert(!strcmp(token, "{"));
    while (true) {
        getToken(token);
        if (strcmp(token, "reflectColor") == 0) {
            reflectColor = readVector3f();
        } else {
            assert(!strcmp(token, "}"));
            break;
        }
    }
    return new ReflectMaterial(reflectColor);
}

Material *SceneParser::parseRefractMaterial() {
    char token[MAX_PARSER_TOKEN_LENGTH];
    ...
    while (true) {
        getToken(token);
        if (strcmp(token, "refractColor") == 0) {
            refractColor = readVector3f();
        } else if (strcmp(token, "refractIndex") == 0) {
...
Material *SceneParser::parseEmissiveMaterial() {
    ...
    return new EmissiveMaterial(emission);
}

Material *SceneParser::parseGlossyMaterial() {
    char token[MAX_PARSER_TOKEN_LENGTH];
    Vector3f diffuseColor(0.8f, 0.8f, 0.8f);
    Vector3f specularColor(0.5f, 0.5f, 0.5f);
    Vector3f f0(-1.0f, -1.0f, -1.0f);
