...
**程序化贴图 `gen_textures`。** `Texture::generateShowcaseTextures` 用 fbm 噪声生成 `plaster_albedo.bmp`（多层频率灰泥色调）、`plaster_normal.bmp`（高度场 → 法线）、`marble_albedo.bmp`（正弦脉理 + fbm）。`encodeNormal` 将 $(n_x,n_y,n_z)$ 映射到 BMP 字节序 B=$n_z$、G=$n_y$、R=$n_x$，与 `loadBMP` 读回的 `Vector3f(r,g,b)` 及 `sampleNormal` 的 $r\to n_x, g\to n_y, b\to n_z$ 一致。
...
<table border="0" align="center">
  <tr>
    <td align="center">
      <img src="results/texture_cornell_notex.png" width="320" height="320">
      <p style="margin-top:8px;font-size:13px;">
        图 2.2a：Panel A — 经典纯色 Cornell，无法线/纹理。
      </p>
    </td>
    <td align="center">
      <img src="results/texture_cornell.png" width="320" height="320">
      <p style="margin-top:8px;font-size:13px;">
        图 2.2b：Panel B — 灰泥 albedo 纹理；墙仍近似平面 Phong。
      </p>
    </td>
    <td align="center">
      <img src="results/texture_cornell_normal.png" width="320" height="320">
      <p style="margin-top:8px;font-size:13px;">
...
**对比设置**：`testcases/scene_whitted_path_compare.txt`，单 **PointLight** `(0, 1.9, 0)`，1024×1024，`gamma`，**CUDA**（复现省时）。**唯一算法变量**：Whitted vs `path_nee`（均含点光直接光）。

| | Whitted | 路径追踪（`path_nee`） |
|--|---------|------------------------|
| 命令 | `whitted 1 gamma cuda` | `path_nee 128 gamma cuda` |
| 光源 | 同一点光 `(0, 1.9, 0)` | 同一点光（NEE 采样） |
| 全局光照 | 无间接反弹、无红/绿墙色溢 | 有 color bleeding |
| 阴影 | 硬阴影 | 硬直接 + 间接柔化 |
| 焦散 | 确定性锐利亮斑 | MC 柔和、有噪 |
| 全图线性亮度均值 | ≈**0.46** | ≈**0.53** |
| 渲染时间 | ≈**1.0 s**（CPU 参考） | ≈**636 s**（CPU 参考；CUDA 复现快得多） |
...
<table border="0" align="center">
  <tr>
    <td align="center">
      ...
      <p style="margin-top: 8px; font-size: 14px;">
        图 2.6a：Whitted（1 SPP）。点光 Phong 直接光 + 硬阴影；红/绿墙无渗色；玻璃立方体下可见确定性折射焦散。
      </p>
    </td>
    <td align="center">
...
- **高光**：Whitted 镜面球 **锐利反射**、与死黑阴影对比强；`path_nee` 周围漫射面被间接照亮，对比 **略弱**。
- **焦散与噪声**：Whitted 玻璃立方体下地板亮斑 **边界清晰、无噪**；`path_nee` 同位置焦散 **更柔、带 MC 颗粒**（128 SPP 墙/地板/过渡区仍见噪）。

#### 差异原因分析

两图差异源于 **Whitted 递归终止** 与 **路径积分 + NEE** 的不同物理近似，而非参数错误。

**1. 递归策略 → 有无 GI。** Whitted 漫反射命中后 **终止**，`shadeDiffuse` 仅累加点光 Phong **直接光**；镜面/玻璃仍 **确定性** 追反射/折射子射线。`path_nee` 在命中点 NEE 注入直接光后，按余弦半球 **BRDF 继续弹射**（RR 截断）。故 Whitted 无法建立红墙→地板→邻墙的二次反射；`path_nee` 经 albedo 弹射产生 color bleeding 并抬升亮度。
...
> 复现：`./build/PA1-2 testcases/scene_whitted_path_compare.txt … whitted 1 gamma cuda` 与 `path_nee 128 gamma cuda`；场景说明见 `testcases/scene_whitted_path_compare.README.txt`。
...
`path` 模式不启用 NEE，仅靠随机弹射间接命中发光体，方差大、收敛慢（**为何 NEE 能显著加速收敛，见 §2.7.3**）。
...
**对比设置**：`testcases/scene_path.txt`（Cornell + 面光源），`path` / `path_nee` 各 **256 SPP**，`gamma`，**CUDA**（复现省时）；**唯一差异** 为是否启用 NEE。
...
**对比设置**：`testcases/scene_mis_demo.txt`（Cornell + 5 颗 GGX 金属球），SPP=32，gamma，CUDA；三模式仅切换采样策略。

| 模式 | 直接光 | 间接 Emissive | 渲染时间（32 SPP） |
|------|--------|---------------|----------|
...
<img src="results/glossy.png" width="450" style="display:block;margin:0 auto;">
...
**对比设置**：`testcases/scene_path.txt`（Cornell + 面光源），`path_nee 512`，**CUDA**；**唯一差异** 为是否传 `gamma`。
...
<table border="0" align="center">
  <tr>
    <td align="center" width="370">
      <img src="results/accel_cpu_no_omp_path32.png" width="370" style="object-fit:contain;">
      <p style="margin-top:8px;font-size:14px;">
        图：未开启 OpenMP，单线程 CPU 渲染，耗时 <strong>2063.09 s</strong>。
      </p>
    </td>
    <td align="center" width="370">
      <img src="results/accel_cpu_omp_path32.png" width="370" style="object-fit:contain;">
      <p style="margin-top:8px;font-size:14px;">
        图：开启 OpenMP 多线程，画质统计一致（MC 噪声下像素非逐点相同），耗时 <strong>418.83 s</strong>。
      </p>
    </td>
  </tr>
</table>
...
./build/PA1-2 testcases/scene_classic_mis.txt output/final/classic_mis.bmp \
    path_mis 15000 gamma cuda dispersion   # GPU；若 OOM 则用 omp 代替 cuda
...
*图 2.15a：经典 Cornell — `path_mis` 15000 SPP，`gamma` + `cuda` + `dispersion`（色散仅作用于玻璃立方体，效果不是很明显；球体坐标见 `testcases/scene_classic_mis.README.txt`）。*
...
| 纹理 A / B / C | §2.2.3 | 图 2.2a–c |
| BVH ON / OFF | §2.3.3 | 图 2.3a–b |
| NEE vs 无 NEE | §2.7.3 | 图 2.7a–b |
| BRDF / NEE / MIS 三模式 | §2.8.3 | 图 2.8a–c |
| Whitted vs Path | §2.6.2 | 图 2.6a–b（Whitted vs `path_nee`，同点光） |
| GGX 光泽 | §2.9.3 | 图 2.9a |
| 色散 | §2.10.3 | 图 2.10a |
| Gamma 开/关 | §2.11.3 | 图 2.11a–b |
| OpenMP 串行/并行 | §2.12.3 | `accel_cpu_no_omp_path32.png`、`accel_cpu_omp_path32.png` |
| CUDA vs CPU | §2.13.3 | Whitted：`accel_cpu_bunny_whitted.png` / `accel_gpu_bunny_whitted.png`；`path_nee`：`accel_cpu_bunny_path64.png` / `accel_gpu_bunny_path64.png` |
| 抗锯齿 SPP 1/16 | §2.14.3 | 图 2.14a–b |
| **综合展示** | §2.15 | 图 2.15a（`classic_mis`） |
...
> **复现策略**：除 §2.12（OpenMP 串行 vs 并行）、§2.13（CPU vs CUDA 计时）、§2.2（纹理仅 CPU）外，**一律加 `cuda`** 以节省墙钟时间；画质与算法对比结论不变。

```bash
# 在 PA1-2 项目的 code/ 目录下执行（若终端已在 code/ 内则勿再 cd）
cmake --build build -j$(nproc)
mkdir -p output/report results

# —— Whitted / Path 对比（§2.6.2，点光源，CUDA）——
...
# —— NEE 对比（§2.7.3，面光源 scene_path.txt，256 SPP，CUDA）——
...
./build/PA1-2 testcases/scene_dispersion.txt output/diag/dispersion_before.bmp path_nee 10000 gamma cuda 
./build/PA1-2 testcases/scene_dispersion.txt output/diag/dispersion_after.bmp path_nee 10000 gamma dispersion cuda
