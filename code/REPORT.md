...
> | §2.2 | 纹理 / 法线贴图 |
> | §2.3 | BVH 加速 |
> | §2.4–§2.10 | Whitted、路径追踪、Whitted/Path 对比、NEE、MIS、GGX、色散 |
> | §2.11 | Gamma 校正 |
> | §2.12 | CPU 加速（OpenMP） |
> | §2.13 | GPU 加速（CUDA） |
> | §2.14 | 抗锯齿（AA） |
...
![Panel A：无纹理](results/texture_cornell_notex.png)

*图 2.2a：Panel A — 经典纯色 Cornell，无法线/纹理。*

![Panel B：仅 albedo 纹理](results/texture_cornell.png)

*图 2.2b：Panel B — 灰泥 albedo 纹理；墙仍近似平面 Phong。*

![Panel C：albedo + 法线贴图](results/texture_cornell_normal.png)

*图 2.2c：Panel C — 法线扰动改变每像素法线；Whitted 点光下高光与阴影边界随 bump 起伏。*

![三面板横向拼接](results/texture_showcase.png)
...
<table border="0" align="center">
  <tr>
    <td align="center">
      <img src="results/bvh_bunny_on.png" width="400" height="400">
      <p style="margin-top:8px;font-size:14px;">
        图 2.3a：BVH <strong>开启</strong>。橙色 Bunny 正确可见；GPU 栈遍历 BVH。
      </p>
    </td>
    <td align="center">
...
- 命中 **EmissiveMaterial** 时返回 `throughput × emission`（NEE 开启时间接路径不重复计光，见 §2.7）。
...
**路径追踪** 在同一命中点按 BRDF **随机** 采样出射方向，递归估计渲染方程，自然包含 **间接光**（颜色渗透、软阴影等），但有限 SPP 下存在 MC 噪声。

**公平对比前提**：两模式必须使用 **同一场景、同一光源几何**。本实验采用 `testcases/scene_path.txt`：Cornell Box（五球 + 贴地玻璃立方体），天花板为 **EmissiveMaterial 三角形** + 两个 **AreaLight**（与发光几何重合），1024×1024。

**旧版异常现象（已修复）**：Whitted 图整体极暗，但 **玻璃立方体前方地板** 出现孤立亮斑。根因是 Whitted 阴影检测函数 `isInShadow` 将 **EmissiveMaterial 当作遮挡物**：向 AreaLight 中心发射的阴影射线先命中天花板发光三角形，被误判为「在阴影中」，导致 **所有漫反射面的 Phong 直接光≈0**。此时仅 **镜面/折射递归路径** 能直接命中发光体并返回 `emission`——玻璃立方体将天花板光 **折射/反射** 到地板，形成局部亮斑，造成「几乎无光 + 地板异常亮」的假象。NEE 使用的 `isSegmentOccluded` 早已对 EMISSIVE 放行，故 Path/NEE 不受此 bug 影响。

修复后：`isInShadow` 与 `isSegmentOccluded` 一致，**发光体不遮挡阴影射线**（CPU `raytracer.hpp`、GPU `cuda_path_tracer.cu` 均已同步）。
...
| `include/light.hpp` | `AreaLight::getIllumination` | Whitted 下将面光 **近似** 为指向三角形中心的方向 + 常数 `color`（非面积积分） |
| 同上 | `castRayPath` | 路径追踪：BRDF 半球采样 + RR；无 NEE 时直接光仅靠随机弹射命中发光体 |
| `src/cuda_path_tracer.cu` | `castRayWhitted`、`isInShadow` | GPU Whitted 镜像逻辑 |
| `src/main.cpp` | CLI | `whitted` 默认 SPP=1；`path` 默认 SPP=64 |

**Whitted 与 Path 的直接光差异（本场景）**

| 环节 | Whitted | `path`（无 NEE） |
|------|---------|------------------|
| 漫反射直接光 | Phong + 单方向阴影（AreaLight → 中心） | 无显式光源采样 |
| 面光可见性 | 修复后阴影射线可「看见」发光三角形 | 需随机弹射命中 0.16² 小面积发光体 |
| 间接光 | **无** | 多次 BRDF 弹射（红/绿墙色溢） |
| 玻璃焦散 | 确定性折射路径 | MC 估计，更软、带噪 |

#### 2.6.3 实验分析与对比

**对比设置**：`testcases/scene_path.txt`，1024×1024，gamma，OpenMP（CPU）。

| | Whitted | 路径追踪 |
|--|---------|----------|
| 命令 | `whitted 1 gamma omp` | `path 128 gamma omp` |
| 直接光 | Phong + AreaLight（单样本近似） | 无 NEE，仅靠随机弹射 |
| 全局光照 | 无间接反弹 | 红/绿墙颜色渗透 |
| 噪声 | 无（确定性） | 128 SPP 仍有颗粒 |
| 渲染时间 | ≈1.0 s | ≈783 s |

<table border="0" align="center">
  <tr>
    <td align="center">
      <img src="results/whitted_compare_whitted.png" width="400" height="400">
      <p style="margin-top: 8px; font-size: 14px;">
        图 2.6a：Whitted（1 SPP）。修复阴影 bug 后 Phong 直接光正常；无间接色溢；玻璃下可见确定性焦散。
      </p>
    </td>
    <td align="center">
      <img src="results/whitted_compare_path.png" width="400" height="400">
      <p style="margin-top: 8px; font-size: 14px;">
        图 2.6b：路径追踪 <strong>无 NEE</strong>（128 SPP）。间接光与软阴影可见，但因无 NEE 直接光方差大，整体仍偏暗（见 §2.7.3）。
      </p>
    </td>
  </tr>
</table>
...
1. **全局光照**：Path 图红/绿墙向邻面 **渗色**；Whitted 墙面交界硬切、无渗透——Whitted 在漫反射面终止递归。
2. **亮度与噪声**：Whitted 无 MC 噪声、直接光稳定；Path 无 NEE 时大量样本未命中发光体，均值偏低（平均亮度 ≈0.38 vs Whitted ≈0.65），同时可见颗粒。
3. **阴影**：Whitted 对 AreaLight 仅用 **指向中心的一条阴影射线**，阴影边缘硬；Path 经多次弹射形成更软的半影（仍带噪）。
4. **焦散**：Whitted 玻璃立方体下 **清晰亮斑**（折射路径命中发光天花板）；Path 同位置斑更 **模糊、随机**。
5. **旧 bug 复盘**：修复前 Whitted 漫反射 direct≈0，仅玻璃 **镜面/折射** 路径贡献 → 全场景暗 + 立方体前地板 **孤立亮斑**；与 Path 的「整体偏暗但均匀有光」完全不同，**不能** 作为算法差异解读。
6. **天花板「重影」**：主因是右上角 **镜面球** 将天花板面光 **反射** 到像面，与真实发光区在屏幕上 **位置接近**（非 emissive 双重计光）。另：`scene_path.txt` 为路径追踪 NEE 将发光四边形拆成 **两片 AreaLight 三角形**；Whitted Phong 若对两片各算一次会直接光 **×2**（已用 `isDuplicateSplitAreaLight` 去重，仅计一片）。发光区外围深灰环来自非发光天花板平面（Material 1）的 Phong 直接光，属 Whitted 面光近似下的 **预期** 几何外观，非 Path 图那种 MC 噪声重影。

> 经典 Whitted 演示亦可用 `scene_whitted.txt`（**点光源**）；本对比 deliberately 使用 `scene_path.txt` 以强制 **同几何、同面光布局**。Whitted 对面光采用 Phong + 中心方向近似，非面积积分；完整面光直接光估计见 §2.7 NEE。
...
<table border="0" align="center">
  <tr>
    <td align="center">
      ...
      <p style="margin-top: 8px; font-size: 14px;">
        图 2.7a：路径追踪 <strong>无 NEE</strong>（64 SPP，`scene_path.txt`）。直接光仅当随机弹射恰好命中发光三角形时出现；发光面积占比极小，方差极大。
      </p>
    </td>
    <td align="center">
      <img src="results/path_nee_cornell.png" width="400" height="400">
      <p style="margin-top: 8px; font-size: 14px;">
        图 2.7b：路径追踪 <strong>含 NEE</strong>（64 SPP，同场景）。漫反射/光泽命中点显式向 AreaLight 采样 + 阴影测试；暗部被稳定照亮，收敛更快。
      </p>
    </td>
  </tr>
</table>
...
<table border="0" align="center">
  <tr>
    <td align="center">
      <img src="results/mis_compare_brdf_32.png" width="320" height="320">
      <p style="margin-top:8px;font-size:13px;">
        图 2.8a：`path 32 gamma cuda`。纯 BRDF 采样、无 NEE；天花板面光极难被随机命中，整体极暗，仅强镜面反射可见。
      </p>
    </td>
    <td align="center">
      <img src="results/mis_compare_nee_32.png" width="320" height="320">
      <p style="margin-top:8px;font-size:13px;">
        图 2.8b：`path_nee 32 gamma cuda`。NEE 稳定照亮场景；光泽球 + 小面光在掠射角仍有 firefly。
      </p>
    </td>
    <td align="center">
      <img src="results/mis_compare_mis_32.png" width="320" height="320">
      <p style="margin-top:8px;font-size:13px;">
        图 2.8c：`path_mis 32 gamma cuda`。NEE 与 BRDF 采样经 power MIS 合并；高光区 firefly 减少，间接 Emissive 与 NEE 一致估计。
      </p>
    </td>
  </tr>
</table>
...
1. **`path` vs `path_nee`**：与 §2.7.3 结论一致——无 NEE 时直接光方差主导，32 SPP 仍大面积欠曝；NEE 将低概率事件转为 $O(1)$ 估计。
...
<table border="0" align="center">
  <tr>
    <td align="center">
      <img src="results/path_nee_nogamma512.png" width="400" height="400">
      <p style="margin-top:8px;font-size:14px;">
        图 2.11a：<strong>无 gamma</strong>。线性 radiance 直接映射 8-bit；暗部细节难辨，整体偏暗。
      </p>
    </td>
    <td align="center">
      <img src="results/path_nee_512.png" width="400" height="400">
      <p style="margin-top:8px;font-size:14px;">
        图 2.11b：<strong>有 gamma</strong>。保存前 $C^{1/2.2}$；中间调与高光更接近日常看图体验。
      </p>
    </td>
  </tr>
</table>
...
**OpenMP 加速对比 A（路径追踪）**：`testcases/scene_path.txt`（1024²），`path_nee 32`，gamma，**CPU only**（不加 `cuda`）。

| 配置 | 渲染时间 | 输出文件 |
|------|----------|----------|
| 无 OpenMP | **2063.09 s** | `cpu_no_omp_path32.bmp` |
| 开启 OpenMP（10 线程） | **418.83 s** | `cpu_omp_path32.bmp` |
| 加速比 | **4.93×** | |

![无 OpenMP（path_nee 32 SPP）](results/accel_cpu_no_omp_path32.png)

*图：未开启 OpenMP，单线程 CPU 渲染，耗时 **2063.09 s**。*

![开启 OpenMP（path_nee 32 SPP）](results/accel_cpu_omp_path32.png)

*图：开启 OpenMP 多线程，画质统计一致（MC 噪声下像素非逐点相同），耗时 **418.83 s**。*

**OpenMP 加速对比 B（Whitted 快速验证）**：`testcases/scene_whitted.txt`（1024²），`whitted 1`，gamma，CPU。

| 配置 | 渲染时间 | 输出文件 |
|------|----------|----------|
| 无 OpenMP | **17.38 s** | `cpu_no_omp_whitted.bmp` |
| 开启 OpenMP（10 线程） | **2.68 s** | `cpu_omp_whitted.bmp` |
| 加速比 | **6.49×** | |

<table border="0" align="center">
  <tr>
    <td align="center">
      <img src="results/accel_cpu_no_omp_whitted.png" width="400" height="400">
      <p style="margin:8px 0 0; font-size:14px;">
        图：未开启 OpenMP，Whitted 单线程渲染，耗时 <strong>17.38 s</strong>。
      </p>
    </td>
    <td align="center">
      <img src="results/accel_cpu_omp_whitted.png" width="400" height="400">
      <p style="margin:8px 0 0; font-size:14px;">
        图：开启 OpenMP 多线程，<strong>像素完全一致</strong>（PIL diff = 0），耗时 <strong>2.68 s</strong>。
      </p>
    </td>
  </tr>
</table>
...
**CUDA 加速对比 A（Whitted，512² Bunny 盒）**：`testcases/scene_bvh_bunny.txt`，`whitted 1`，gamma。

| 配置 | 渲染时间 | 输出文件 |
|------|----------|----------|
| CPU + OpenMP（10 线程） | **20.15 s** | `cpu_bunny_whitted.bmp` |
| CUDA + BVH | **0.0029 s** | `gpu_bunny_whitted.bmp` |
| 加速比 | **6904×** | |

<table border="0" align="center">
  <tr>
    <td align="center">
      <img src="results/accel_cpu_bunny_whitted.png" width="400" height="400">
      <p style="margin-top:8px;font-size:14px;">
        图：CPU OpenMP 渲染 Bunny 盒，耗时 <strong>20.15 s</strong>。
      </p>
    </td>
    <td align="center">
      <img src="results/accel_gpu_bunny_whitted.png" width="400" height="400">
      <p style="margin-top:8px;font-size:14px;">
        图：GPU 渲染，画质与 CPU <strong>几乎逐像素相同</strong>（PIL diff = 6 像素、±1 色阶），耗时 <strong>0.0029 s</strong>。
      </p>
    </td>
  </tr>
</table>

**CUDA 加速对比 B（路径追踪 + BVH）**：`testcases/scene_bvh_bunny.txt`（512²），`path_nee 64`，gamma。

| 配置 | 渲染时间 | 输出文件 |
|------|----------|----------|
| CPU + OpenMP（10 线程） | **5315.50 s** | `cpu_bunny_path64.bmp` |
| CUDA + BVH | **0.63 s** | `gpu_bunny_path64.bmp` |
| 加速比 | **8458×** | |

<table border="0" align="center">
  <tr>
    <td align="center">
      <img src="results/accel_cpu_bunny_path64.png" width="400" height="400">
      <p style="margin-top:8px;font-size:14px;">
        图：CPU OpenMP 路径追踪（无 mesh BVH），耗时 <strong>5315.50 s</strong>（≈88 min）。
      </p>
    </td>
    <td align="center">
      <img src="results/accel_gpu_bunny_path64.png" width="400" height="400">
      <p style="margin-top:8px;font-size:14px;">
        图：GPU 栈式 BVH + 像素并行，同算法统计一致（RNG 不同：LCG vs curand），耗时 <strong>0.63 s</strong>。
      </p>
    </td>
  </tr>
</table>
...
<table border="0" align="center">
  <tr>
    <td align="center">
      <img src="results/aa_before.png" width="400" height="400">
      <p style="margin-top:8px;">图 2.14a：<strong>SPP=1</strong>。每像素单条过中心光线；球缘、玻璃棱边可见 jaggies。</p>
    </td>
    <td align="center">
      <img src="results/aa_after.png" width="400" height="400">
      <p style="margin-top:8px;">图 2.14b：<strong>SPP=16</strong>。像素内 16 个 hash 抖动子样本取平均；几何边缘显著平滑。</p>
    </td>
  </tr>
</table>
...
<table border="0" align="center">
  <tr>
    <td align="center">
      <img src="results/aa_before_zoom.png" width="400" height="400">
      <p style="margin-top:8px;">图 2.14c：<strong>SPP=1</strong></p>
    </td>
    <td align="center">
      <img src="results/aa_after_zoom.png" width="400" height="400">
      <p style="margin-top:8px;">图 2.14d：<strong>SPP=16</strong></p>
    </td>
  </tr>
</table>
...
在 `scene_path.txt` 经典布局上仅改两处材质，用 `path_mis` 渲染：

| 球体 | 坐标 (x,y,z) | 原材质 | 本场景 |
|------|--------------|--------|--------|
| 左上高球 | (-0.52, 1.12, -0.42) | 蓝色漫反射 | **色散玻璃** IOR 1.50，`dispersionDelta 0.10` |
| 右下低球 | (0.60, 0.38, 0.40) | 完美镜面 | **金色 GGX 光泽** roughness 0.18 |
| 其余 | — | 红/绿漫反射、右上镜面球、玻璃立方体 | 不变 |

> 原场景「蓝球」在几何上位于相机**左上**（x<0），非右上；右上高球仍为镜面。

```bash
./build/PA1-2 testcases/scene_classic_mis.txt output/final/classic_mis.bmp \
    path_mis 128 gamma cuda dispersion   # GPU；若 OOM 则用 omp 代替 cuda
python3 -c "from PIL import Image; Image.open('output/final/classic_mis.bmp').save('output/final/classic_mis.png')"
cp output/final/classic_mis.png results/classic_mis.png
```

![经典 Cornell MIS](results/classic_mis.png)

*图 2.15b：经典 Cornell — `path_mis` 64 SPP，`gamma` + `omp` + `dispersion`（CPU；球体坐标说明见 `testcases/scene_classic_mis.README.txt`）。*
...
**分辨率约定**：算法对比单帧（§2.3–§2.8、§2.11 等）统一 **1024×1024**；§2.13 CUDA/CPU Bunny 性能对比刻意保持 **512×512**（`path_nee 64` 在 CPU 上 1024² 需数小时）。Markdown 并排图统一 `width=400 height=400`（三列 MIS 为 320×320），避免预览尺寸不一致。**若组内像素尺寸已相同但观感仍不同，多为画面内容/亮度差异，而非分辨率不同。**
...
| 局限 | 说明 |
|------|------|
| Whitted 面光近似 | Whitted 对 AreaLight 用 Phong + 指向三角形中心，非面积积分；与 `path_nee` 的立体角采样不等价 |
...
python3 -c "from PIL import Image; Image.open('output/report/whitted_path_compare/whitted.bmp').save('output/report/whitted_path_compare/whitted.png'); Image.open('output/report/whitted_path_compare/path.bmp').save('output/report/whitted_path_compare/path.png')"

# —— NEE 对比（§2.7.3）——
./build/PA1-2 testcases/scene_path.txt output/report/path_nee_cornell.bmp path_nee 128 gamma omp
